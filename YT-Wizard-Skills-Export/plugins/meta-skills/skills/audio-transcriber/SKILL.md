...
Use Python or bash with AI model (OpenAI-compatible or local) for intelligent summarization:
...
def generate_summary(segments, max_paragraphs=5):
    """Create executive summary using AI (OpenAI API or local model)."""
    
    full_text = " ".join([s["text"] for s in segments])
    
    # Use Chain of Density approach (from built-in prompt optimization frameworks)
    summary_prompt = f"""
    ...
    # Call AI model (placeholder - user can integrate OpenAI API via OPENAI_API_KEY or a local model)
...
2. **Automatically improve with built-in prompt optimization (if available):**
   ```bash
   🔧 Melhorando prompt com built-in prompt optimization...
   [Invokes: OpenAI API -p "melhore este prompt: {user_prompt}"]
   ```
...
def process_with_llm(transcript, prompt, model_name="gpt-4.1-mini"):
    ...
    with Progress(
        ...
        progress.add_task(
            description=f"Processing with {model_name}...",
            total=None
        )
...
This skill is **platform-agnostic** and works in any terminal context where OpenAI API is available. It requires an OpenAI API key for summary generation, following the zero-configuration philosophy.
