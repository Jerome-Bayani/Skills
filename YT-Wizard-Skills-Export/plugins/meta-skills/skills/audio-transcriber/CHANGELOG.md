...
- **Intelligent Prompt Workflow** (Step 3b) - Complete integration with built-in prompt optimization skill
  - **Scenario A**: User-provided prompts are automatically improved with built-in prompt optimization
...
- **LLM Integration** - Process transcripts with the OpenAI API
  - Priority: OpenAI API > transcript-only mode
  - Step 0b: API availability and fallback behavior documented
...
- **Optional:** OpenAI API or OpenAI API for intelligent processing
- **Optional:** built-in prompt optimization skill for automatic prompt generation
